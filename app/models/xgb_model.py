...
class XGBMODEL:
    ...
    def take_prediction_using_xgb_model(self, time_series=None, medal_type=None):
        logging.info('__take_prediction_using_xgb_model__')
        try:
            ...
            xgb_model.fit(train)
            forecast = xgb_model.predict(len(test))
            ...
            return train, test, xgb_model, forecast, forecast_df, dta

        except Exception as e:
            logging.error(f'Error occurred while predicting using XGB Model: {e}')
    ...
    def calculate_errors(self, actual, forecasted, medal_type):
        logging.info("__calculate_errors__")
        try:
            ...
            logging.info(f"Error calculation successful for {medal_type} medals.")
            return metrics

        except Exception as e:
            logging.error(
                f"Error occurred while calculating errors for {medal_type} medals.\n"
                f"Inputs:\n"
                f"Actual: {actual}\n"
                f"Forecasted: {forecasted}\n"
                f"Error: {str(e)}",
    ...
    def filling_missing_dates(self,df=None,medal_type=None):
        logging.info(f"__filling_missing_dates__")
    ...
    def aligning_dates(self,df=None,medal_type=None):
        logging.info("__aligning_dates__")
    ...
    def xgb_future_forecast(self,timeseries=None,horizan=10,medal_type=None):
        logging.info(f"future_predictions_using_xgb_model function called  medal_type : {medal_type}")
        
        xgb_model = XGBModel(lags=12,output_chunk_length=6)
        xgb_model.fit(timeseries)
    ...
    def future_predictions_using_xgb_model(self,country_name=None,forecast_horizon=10):
        logging.info(f"__future_predictions_using_xgb_model__")
        ...
        for index_name, group_name in medal_data:
            if group_name['Code'].values[0] == country_name:
                ...
                gold_forecasted_df = self.xgb_future_forecast(timeseries=time_series_gold,horizan=forecast_horizon,medal_type="Gold_Medals")
                silver_forecasted_df = self.xgb_future_forecast(timeseries=time_series_silver,horizan=forecast_horizon,medal_type="Silver_Medals")
                bronze_forecasted_df = self.xgb_future_forecast(timeseries=time_series_bronze,horizan=forecast_horizon,medal_type="Bronze_Medals")
    ...
    def train_test_xgb_model(self,country_name='USA'):
        logging.info("__train_test_xgb_model__")
        try:

            medal_data = self.summar_dataframe.groupby(['Country_y'])
            file_path = "xgb_results_metrics.csv"
            ...
            for index_name, group_name in medal_data:
                if group_name['Code'].values[0] == country_name:
                    ...
                    train_gold, test_gold, arma_mod30_gold, forecast_gold, forecast_df_gold, dta_gold = self.take_prediction_using_xgb_model(time_series=time_series_gold,
                                                                                                                                            medal_type='Gold_Medals')
                    train_silver, test_silver, arma_mod30_silver, forecast_silver, forecast_df_silver, dta_silver = self.take_prediction_using_xgb_model(time_series=time_series_silver,
                                                                                                                                                        medal_type='Silver_Medals')
                    train_bronze, test_bronze, arma_mod30_bronze, forecast_bronze, forecast_df_bronze, dta_bronze = self.take_prediction_using_xgb_model(time_series=time_series_bronze,
                    ...
                    # logging.info(f"1: General testing dataset : {general_test}")
            ...
            logging.error(f"\nwhat=Taking Prediction from xgb model"
                          f"\nwhere=train_test_xgb_model"
                          f"\nError:{e}",
                          exc_info=True)
