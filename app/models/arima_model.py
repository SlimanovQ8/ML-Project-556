...
class ARIMAMODEL:
    ...
    def take_prediction_using_auto_arima(self, time_series=None, medal_type=None):
        logging.info('take_prediction_using_auto_arima function called')
        try:
            # Split data into train and test
            train, test = time_series.split_after(0.80)
    ...
    def prepare_time_series(self,df=None,full_years_df=None, medal_type=None, frequency='4YS'):
        ...
        try:
            ...
            return None
    def auto_arima_future_forecast(self,timeseries=None,horizan=10,medal_type=None):
    ...
    def train_test_random_forest_model(self,country_name='USA'):
        ...
        try:
            ...
            file_path = "arima_model_results_metrics.csv"
    ...
    def train_test_auto_arima_model(self,country_name='USA'):
        logging.info(f"train_test_auto_arima_model function called  country_name : {country_name}")
        try:

            # Group by 'Country_y'
            medal_data = self.summar_dataframe.groupby(['Country_y'])
            file_path = "auto_arima_results_metrics.csv"
            ...
            for index_name, group_name in medal_data:
                if group_name['Code'].values[0] == country_name:
                    ...
                    # Call the prediction function for each medal type
                    train_gold, test_gold, arma_mod30_gold, forecast_gold, forecast_df_gold, dta_gold = self.take_prediction_using_auto_arima(time_series=time_series_gold,
                                                                                                                                            medal_type='Gold_Medals')
                    train_silver, test_silver, arma_mod30_silver, forecast_silver, forecast_df_silver, dta_silver = self.take_prediction_using_auto_arima(time_series=time_series_silver,
                                                                                                                                                        medal_type='Silver_Medals')
                    train_bronze, test_bronze, arma_mod30_bronze, forecast_bronze, forecast_df_bronze, dta_bronze = self.take_prediction_using_auto_arima(time_series=time_series_bronze,
                                                                                                                                                        medal_type='Bronze_Medals')
            ...
            logging.error(f"Error occurred while processing country: {country_name}, error: {str(e)}",exc_info=True)
            return None
    def future_predictions_using_auto_arima_model(self,country_name=None,forecast_horizon=10):
        medal_data = self.summar_dataframe.groupby(['Country_y'])
        ...
        for index_name, group_name in medal_data:
            if group_name['Code'].values[0] == country_name:
                ...
                gold_forecasted_df = self.auto_arima_future_forecast(timeseries=time_series_gold,horizan=forecast_horizon,medal_type="Gold_Medals")
                silver_forecasted_df = self.auto_arima_future_forecast(timeseries=time_series_silver,horizan=forecast_horizon,medal_type="Silver_Medals")
                bronze_forecasted_df = self.auto_arima_future_forecast(timeseries=time_series_bronze,horizan=forecast_horizon,medal_type="Bronze_Medals")
